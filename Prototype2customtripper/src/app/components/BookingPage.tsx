...
export function BookingPage() {
  ...
  const sendBookingToServer = async (payload: any) => {
    try {
      ...
      if (!res.ok) {
        ...
        console.warn("Booking API returned an error", res.status, body);
        return null;
      }

      return await res.json();
    } catch (error) {
      console.warn("Booking API request failed", error);
      return null;
    }
  };
  ...
  const handleConfirmBooking = async () => {
    ...
    // Attempt to persist to the backend as well.
    const serverResult = await sendBookingToServer({
      ...newTrip,
      specialRequests,
    });

    if (serverResult?.id) {
      sessionStorage.setItem("bookingId", serverResult.id.toString());
    }
